...
func GetCrc16(pdata []byte) (fcs []byte) {
	d := getCrc16(pdata)
	rd := make([]byte, 2)
	binary.BigEndian.PutUint16(rd, d)
	return rd
}
...
func HexStringToCrcHexString(hexstr string) (result string) {

	src := []byte(hexstr)
	dst := make([]byte, hex.DecodedLen(len(src)))
	n, _ := hex.Decode(dst, src)
	fmt.Println(dst[:n])
	return fmt.Sprintf("%x", GetCrc16(dst[:n]))
}
